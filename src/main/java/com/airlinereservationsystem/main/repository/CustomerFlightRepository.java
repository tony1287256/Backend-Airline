...
public interface CustomerFlightRepository extends JpaRepository<CustomerFlight, Integer> {
	...
	/*
	 * @Query("SELECT SUM(cf.price) FROM CustomerFlight cf") Double
	 * getTotalIncome();
	 */
	...
	@Query("select cf from CustomerFlight cf where cf.date = ?1")
	List<CustomerFlight> getByDate(LocalDate date);
